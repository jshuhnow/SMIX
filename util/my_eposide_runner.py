...
class MyEpisodeRunner:
    ...
    def get_actions(self):
        pre_transition_data = {
            "state": [self.env.get_state()] * 8,
            "avail_actions": [self.env.get_avail_actions()] * 8,
            "obs": [self.env.get_obs()] * 8
        }
    ...
    def post_action(self, reward, terminated, env_info, actions):
        ...
        post_transition_data = {
            "actions": actions,
            "reward": [(reward,) * 8],
            "terminated": [(terminated != env_info.get("episode_limit", False),) * 8],
        }
